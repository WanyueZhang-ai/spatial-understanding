...
image_paths = ['YOUR_PATH/attention_visualize_cot/images/data-v3_sc3_staging_16-v2/11_14/14_rgb_image.png', 'YOUR_PATHattention_visualize_cot/images/data-v3_sc3_staging_16-v2/11_14/11_rgb_image.png']
...
consistency_cot = "\nBefore answering, compare the two images to identify the same objects across different viewpoints. Analyze how each object's appearance, position, and visibility change due to the viewpoint shift. Use this comparison to resolve spatial ambiguities. Think step by step. Ensure your reasoning is consistent and grounded in visual evidence from all views. Only after this reasoning, choose the correct answer. And you should perform your step-by-step reasoning privately and not reveal it to the user. Only respond with the corresponding letter.\n"
...
messages_query = [{
    ...
    "content": [
        ...
        {"type": "text", "text": f"{question}{consistency_cot}"},
    ],
}]
...
def visualize_attention(img_index, save_path):
    ...
    title_text = f'Question: {question}{consistency_cot}'.replace('\n', '\\n')
...
def compute_attention_ratio(output, start_pos_list, end_pos_list, label="Query"):
    image_attention_ratios = []
    text_attention_ratios = []

    print(f"==== {label} ====")

    for layer_idx, layer_attn in enumerate(output.attentions):
        # shape: (batch_size=1, num_heads, query_len, key_len)
        attn_weights = layer_attn[0, :, -1, :]  # shape: (num_heads, key_len)
        attn_mean = attn_weights.mean(dim=0)  # (key_len,) => mean over heads

        img_attn_sum = 0.0
        for start, end in zip(start_pos_list, end_pos_list):
            img_attn_sum += attn_mean[start:end].sum().item()

        total_attn_sum = attn_mean.sum().item()
        text_attn_sum = total_attn_sum - img_attn_sum

        img_ratio = img_attn_sum / total_attn_sum
        text_ratio = text_attn_sum / total_attn_sum

        image_attention_ratios.append(img_ratio)
        text_attention_ratios.append(text_ratio)

        print(f"Layer {layer_idx + 1:2d}: Image Attention = {img_ratio*100:.2f}%, Text Attention = {text_ratio*100:.2f}%")

    return image_attention_ratios, text_attention_ratios

_ = compute_attention_ratio(output, start_pos, end_pos, label="Question")

os.makedirs('./output_qwen2.5_VL_7b-3/', exist_ok=True)
visualize_attention(0, './output_qwen2.5_VL_7b-3/qwen2.5_7b_attention_image0-Implicit-Multi-view.png')
visualize_attention(1, './output_qwen2.5_VL_7b-3/qwen2.5_7b_attention_image1-Implicit-Multi-view.png')

os.makedirs('./output_qwen2.5_VL_7b_layers-3/', exist_ok=True)
visualize_attention_layers_separately(0, './output_qwen2.5_VL_7b_layers-3/image0-Implicit-Multi-view')
visualize_attention_layers_separately(1, './output_qwen2.5_VL_7b_layers-3/image1-Implicit-Multi-view')
