            ...
            if position_ids.shape[1] != 1:
                ...
                if mode == "vanilla":
                    ...
                    for i in range(batch_size):
                        idx = torch.randperm(middle_end - middle_start, device=device) + middle_start
                        shuffled_position_ids[i, middle_start:middle_end] = position_ids[i, idx]
                    model_inputs["position_ids"] = shuffled_position_ids
                elif mode == "all_random":
                    batch_size, seq_len = position_ids.shape
                    shuffle_idx = torch.randperm(seq_len)  # shape: [729]
                    shuffled_position_ids = position_ids[:, shuffle_idx]
                    model_inputs["position_ids"] = shuffled_position_ids
                elif mode == "vit_in_one_first":
                    position_ids[:, sys_prompt:-question] = sys_prompt
                    model_inputs["position_ids"] = position_ids
                elif mode == "vit_in_one_last":
                    position_ids[:, sys_prompt:-question] = position_ids.shape[1] - 1 - question
                    model_inputs["position_ids"] = position_ids
                elif mode == "vit_all_plus_num":
                    num = 10
                    position_ids[:, sys_prompt:-question] += num
                    model_inputs["position_ids"] = position_ids
