        ...
        **model_kwargs,
        ...
        while True:
            ...
            if position_ids.shape[1] != 1:
                import os
                mode = os.getenv("LLM_POSITION_MODE", "vanilla")
                # print(mode)
                # mode = "vit_in_one_last"
                sys_prompt = int(os.getenv("SYS_PROMPT", "0"))
                question = int(os.getenv("QUESTION", "0"))
                num_images = int(os.getenv("NUM_IMAGES", "1"))

                if mode == "vanilla":
                    pass

                elif mode == "vit_random":
                    device = position_ids.device
                    batch_size, seq_len = position_ids.shape
                    middle_start = sys_prompt
                    middle_end = seq_len - question
                    if middle_end <= middle_start:
                        raise ValueError("无效区间，middle_end <= middle_start")
                    
                    image_region_length = middle_end - middle_start
                    image_length = image_region_length // num_images
                    
                    shuffled_position_ids = position_ids.clone()
                    for i in range(batch_size):
                        for img_idx in range(num_images):
                            img_start = middle_start + img_idx * image_length
                            img_end = img_start + image_length
                            
                            idx = torch.randperm(image_length, device=device) + img_start
                            shuffled_position_ids[i, img_start:img_end] = position_ids[i, idx]
                    
                    model_inputs["position_ids"] = shuffled_position_ids
                    ...
                    for i in range(batch_size):
                        ...
                        shuffled_position_ids[i, :] = position_ids[i, idx]
                    
                    model_inputs["position_ids"] = shuffled_position_ids

                elif mode == "vit_in_one_first":
                    device = position_ids.device
                    batch_size, seq_len = position_ids.shape
                    middle_start = sys_prompt
                    middle_end = seq_len - question
                    ...
                    modified_position_ids = position_ids.clone()
                    for i in range(batch_size):
                        for img_idx in range(num_images):
                            img_start = middle_start + img_idx * image_length
                            img_end = img_start + image_length
                            modified_position_ids[i, img_start:img_end] = img_start
                    
                    model_inputs["position_ids"] = modified_position_ids

                elif mode == "vit_in_one_last":
                    device = position_ids.device
                    batch_size, seq_len = position_ids.shape
                    middle_start = sys_prompt
                    middle_end = seq_len - question
                    ...
                    for i in range(batch_size):
                        for img_idx in range(num_images):
                            img_start = middle_start + img_idx * image_length
                            img_end = img_start + image_length
                            modified_position_ids[i, img_start:img_end] = img_end - 1
                    
                    model_inputs["position_ids"] = modified_position_ids

                elif mode == "vit_all_plus_num":
                    num = 10
                    device = position_ids.device
                    batch_size, seq_len = position_ids.shape
                    middle_start = sys_prompt
                    ...
                    for i in range(batch_size):
                        for img_idx in range(num_images):
                            img_start = middle_start + img_idx * image_length
                            img_end = img_start + image_length
                            modified_position_ids[i, img_start:img_end] += num
